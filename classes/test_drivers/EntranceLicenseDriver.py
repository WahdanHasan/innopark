...
def StartEntranceLicenseDetector(license_frames_request_queue):
    ...
    license_detector.InitializeCameras(["D:\\DownloadsD\\License_Footage\\Entrance_Bottom_Simulated_2.mp4", 0],
                                       ["D:\\DownloadsD\\License_Footage\\Entrance_Top.mp4", 1])
    license_detector_process = multiprocessing.Process(target=license_detector.Start(license_frames_request_queue))
...
def StartProcessingLicenseFrames(license_frames_request_queue):
    ...
    license_processing_process = multiprocessing.Process\
        (target=license_processing_frames.Start(license_frames_request_queue))
