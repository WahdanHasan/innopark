...
class MyAlgorithm_dynamicROI(threading.Thread):
    global lin
    ...
    def execute(self):
        ...
        _, frame_final = self.camera.read()
        ...
        while (stop_button == False):
            ...
            _, frame_final2 = self.camera.read()
            ...
            for i, (f2, f1) in enumerate(zip(p1, p0)):
                a, b = f2.ravel()
                c, d = f1.ravel()
                cv2.circle(frame_final_cut2, (int(a), int(b)), 5, (255, 255, 0), -1)
                cv2.circle(frame_final_cut2, (int(c), int(d)), 5, (255, 0, 0), -1)
                cv2.line(frame_final_cut2, (int(a), int(b)), (int(c), int(d)), (0, 0, 255), 2)

            cv2.rectangle(frame_final_cut2, (np.int0(minX), np.int0(minY)), (np.int0(maxX), np.int0(maxY)), (0, 255, 0),
                          2)
            ...
            index2 = 0
            for p in (p0):
                if (p[0][0] < (np.int0(minX) - 2) or p[0][0] > (np.int0(maxX) + 2)) or (
                        p[0][1] < (np.int0(minY) - 2) or p[0][1] > (np.int0(maxY) + 2)):
                    p0 = np.delete(p0, index2, axis=0)
                else:
                    index2 = index2 + 1

            if len(p0) < 10:
                cv2.rectangle(frame_final_cut2, (np.int0(minX), np.int0(minY)), (np.copy(maxX), np.int0(maxY)),
                              (0, 255, 0), 2)
                p0 = cv2.goodFeaturesToTrack(frame_final_cut_gray, 100, 0.01, 10, None, None, 7)
                index3 = 0
                for g in (p0):
                    if ((g[0][0] < (minX - 20)) or (g[0][0] > (maxX + 20))) or (
                            (g[0][1] < (minY - 20)) or (g[0][1] > (maxY + 20))):
